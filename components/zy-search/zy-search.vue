	...
	<view>
		<view class="search">
			...
			<template v-if="isFocus">
				<input focus type="text" value="" confirm-type="search" @confirm="searchStart()" :placeholder="searchPlaceholder" v-model.trim="searchText"/>
			</template>
			<template v-else>
				<input maxlength="50" type="text" value="" confirm-type="search" @confirm="searchStart()" placeholder="请输入关键词搜索" v-model.trim="searchText"/>
			</template>
		...
		methods: {
			searchStart: function() {	//触发搜索
				...
				if (_this.searchText == '') {
					...
					this.$emit('onConfirm', _this.searchText) 
				}
			},
